...
<html lang="en">
<head>
  <title><?php echo $app_name; ?> - Receiving Reports</title>
  <meta charset="utf-8">
		  	...
		  	<ul class='nav navbar-nav navbar-right'>
				...
				<li>
					<a href='#' role='button' 
					  ...
					  data-content = "
						<?php if($type=='admin'){?><?php } ?><a href='settings' class = 'list-group-item'><span class='glyphicon glyphicon-cog'></span> Settings</a>
...
<div class="container-fluid">
  <div class='row'>
	...
	if($reports=='1'){
		echo "<center><h3>Receiving Cost in <br>".date("F d, Y",strtotime($f))." - ".date("F d, Y",strtotime($t))."</h3></center>";
	?>
	...
	<button class="btn btn-primary prints" onclick="window.print()"><span class="glyphicon glyphicon-print"></span> Print</button>
		...
		$query = "SELECT * FROM tbl_orders_receiving WHERE date_received BETWEEN '".strtotime($f)."' AND '".strtotime($t)."'";
		...
		if(mysql_num_rows($salesquery)!=0){
			while($row=mysql_fetch_assoc($salesquery)){
				...
				$comment_query = mysql_query("SELECT * FROM tbl_orders_receiving WHERE orderID='$orderID'");
				...
				echo "
				<tr title='$comments'>
					<td><a href='receiving-re?id=$orderID' target='_blank'>R".sprintf("%06d", $orderID)."</a></td>
					<td>".ucfirst($mode)."</td>
					<td>$time_received</td>
		...
		<tfoot>
			<?php
			if(mysql_num_rows($salesquery)!=0){
				$incomequery = mysql_query("SELECT SUM(total_cost) as total_expenses FROM tbl_orders_receiving WHERE date_received BETWEEN '".strtotime($f)."' AND '".strtotime($t)."'");
				while($incomerow=mysql_fetch_assoc($incomequery)){
					$total_expenses = $incomerow["total_expenses"];
				}
