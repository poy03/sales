...
<html lang="en">
<head>
  <title><?php echo $app_name; ?> - Sales Reports per Category</title>
  <meta charset="utf-8">
...
<div class="container-fluid">
  <div class='row'>
	...
	<!-- <center><h3 class='page' style="text-align:center"><?php echo $app_company_name; ?></h3></center> -->
	<?php
	...
	if($reports=='1'){
		...
		<button onclick="window.print()" class="btn btn-primary prints"><span class="glyphicon glyphicon-print"></span>&nbsp;&nbsp; Print </button>
			<table class="table table-hover">
				...
				<tbody>
					...
					$query = "SELECT * FROM tbl_purchases";

					$items_of_category_query = mysql_query("SELECT * FROM tbl_items WHERE category='".mysql_real_escape_string(htmlspecialchars(trim($_GET["by"])))."' AND deleted='0'");
					if(mysql_num_rows($items_of_category_query)!=0){
						$query.=" WHERE (";
						$i = 0;
						while($items_list = mysql_fetch_assoc($items_of_category_query)){
							$itemID_in = $items_list["itemID"];
							
							if($i==0){
								$query .=" itemID='".$itemID_in."'";
							}else{
								$query .=" OR itemID='".$itemID_in."'";
							}
							$i++;
						}
						$query.=")";
					...
					if(mysql_num_rows($sales_items_query)!=0){
						while($sales_items_row=mysql_fetch_assoc($sales_items_query)){
							$item_data = mysql_fetch_assoc(mysql_query("SELECT * FROM tbl_items WHERE itemID='".$sales_items_row["itemID"]."' AND deleted='0'"));

							$total_cost_data = mysql_fetch_assoc(mysql_query("SELECT SUM(costprice) as total_costprice FROM tbl_purchases WHERE itemID='".$sales_items_row["itemID"]."' AND deleted='0'"));
							$total_selling_price_data = mysql_fetch_assoc(mysql_query("SELECT SUM(price) as total_selling_price FROM tbl_purchases WHERE itemID='".$sales_items_row["itemID"]."' AND deleted='0'"));
							echo "
							<tr>
				...
				<tfoot>
						...
						$sales_items_query = mysql_query($query);
						if(mysql_num_rows($sales_items_query)!=0){
							echo "<tr><td colspan='5' style='text-align:right'>Total Cost:</td><th style='text-align:right;'>".number_format($total_cost_data["total_costprice"],2)."</th></tr>";
							echo "<tr><td colspan='5' style='text-align:right'>Total Sales:</td><th style='text-align:right;'>".number_format($total_selling_price_data["total_selling_price"],2)."</th></tr>";							
						}
