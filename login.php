...
<div class="container-fluid">
  <div class='row'>
  	<div class='col-md-10 col-md-offset-2' prints'>
	...
	if(isset($_POST["login"])){
		...
		$query = mysql_query("SELECT * FROM tbl_users WHERE username='$username' AND deleted = '0'");
		$err= "";
		if(mysql_num_rows($query)!=0){
			...
			if(md5($password)==$dbpassword){
				...
				if(isset($remember)){
					$cookie_name = "LOGGED";
					$cookie_value = md5(md5(md5($accountID)))."c".$accountID;
					setcookie($cookie_name, $cookie_value, time() + (86400 * 30), "/");
				}
				header("location:index");
			}else{
				$err = "Invalid username of password";
			}
		}else{
			$err = "Invalid username of password";
		}
